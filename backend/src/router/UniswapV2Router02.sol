...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function _addLiquidity(
        address tokenA,
        address tokenB,
        uint amountADesired, //put by the user
        uint amountBDesired, //put by the user
        uint amountAMin, //put by router via ui
        uint amountBMin //put by router via ui to avoid big loss or reserve moving too much
    ...
    function removeLiquidity(
        ...
        (amountA, amountB) = tokenA == token0
            ? (amount0, amount1)
            : (amount1, amount0); //to assign right amount to right token
        require(
            amountA >= amountAMin,
            "UniswapV2Router: INSUFFICIENT_A_AMOUNT"
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(
            UniswapV2Library.pairFor(factory, path[0], path[1]), amountIn
        ));
